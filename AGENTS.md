...
Require the worker to follow the version-independent validation gate-response policy embedded by `bin/fm-brief.sh`; the installed no-mistakes skill and live `axi` help own version-specific mechanics.
...
A worker whose pushed PR awaits merge is a declared external wait, and `bin/fm-pr-check.sh` declares it automatically.
...
They also own the ship brief's claim-proof discipline, whose gate-dependent parts are mode-conditional: only no-mistakes briefs have a review gate, so only they exempt a listed finding's own defect class from class-generalization escalation and cite gate-response rules by number.
...
- `decision-hold-lifecycle` - load before treating an investigation or visual review as complete, before ending a visual review that exposed a decision, on a `captain-ruling` or `captain-ruling-revision` `check:` wake, on any `captain-ruling-error` `check:` wake, and when recording or routing the captain's answer.
