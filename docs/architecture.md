...
PR-based task merges go through `bin/fm-pr-merge.sh`, which records `pr=` and any available `pr_head=` through `bin/fm-pr-check.sh`, then reads forge merge truth so a PR the forge already reports merged is reconciled by emitting the missing merged work stage instead of being merged again; its header owns that merge-state contract.
