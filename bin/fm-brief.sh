...
2. Run \`no-mistakes doctor\`; if it reports the repo is not initialized here, run \`no-mistakes init\`."
   ...
   A defect class that a listed finding does belong to is not this case: it stays in the round and you close it there, as far as rules 2 and 3 make its instances fix-eligible; a same-class instance they defer stays a follow-up and the class still counts as closed, and one whose deferral would defeat the purpose of the change goes to firstmate as rule 5 requires.'
    DOD=$(cat <<EOF
# Definition of done
The task is complete only when committed on your branch.
