...
# a default line rather than a placeholder, since most tasks have none. That default
# states only that none are RECORDED, which stays true when a harvested decision is
# still captain-held and unanswered.
...
# must read as a complete instruction, never as a leaked placeholder. The default speaks
# only about this brief's record, never claiming the harvest found nothing, so it stays
# true for a task dispatched while a harvested decision is still captain-held.
# The standing instruction below is always present and is not firstmate-editable.
PREFLIGHT_DECISIONS=${FM_PREFLIGHT_DECISIONS:-No pre-flight decisions are recorded for this task.}
...
6. If a decision belongs to a human (product choices, destructive actions),
   ...
   Keep that append to a single short line, exactly as rule 4 requires: firstmate's monitoring reads
   the last status line, so a multi-line append hides the decision from it. Put the full framing in
   your pane output or the report instead, which firstmate reads on wake:
   the options and their tradeoffs, your recommendation, and a one-line blast radius -
   what breaks if the call is wrong, and what would catch it if it does.
   That framing sharpens the decision and must never delay reporting it.
...
2. Run \`no-mistakes doctor\`; if it reports the repo is not initialized here, run \`no-mistakes init\`."
    ...
    CLAIM_SCOPE_NOTE='Both preserve the validation gate-response contract below: escape testing validates only an already-scoped fix and adds no finding or surface (rule 3), and naming an unlisted generalization authorizes no follow-up fixing or scope growth into an unrelated defect class or unrelated surface (rule 5). The defect class a listed finding belongs to is never such a generalization - rule 5 requires you to close that class in the same round, within the fix-eligible boundary rules 2 and 3 set, so escalating that class instead of closing it there is the instance-only closure rule 5 forbids. Two things are not that forbidden closure and stay required: a same-class instance rules 2 or 3 defer remains a follow-up and the class still counts as completely closed, and a deferral rule 5 calls purpose-defeating goes to firstmate rather than into a silent follow-up.'
   ...
   A defect class that a listed finding does belong to is not this case: it stays in the round and you close it there, as far as gate-response rules 2 and 3 make its instances fix-eligible; a same-class instance they defer stays a follow-up and the class still counts as closed, and one whose deferral would defeat the purpose of the change goes to firstmate as gate-response rule 5 requires.'
    DOD=$(cat <<EOF
# Definition of done
The task is complete only when committed on your branch.
...
6. If a decision belongs to a human (product choices, destructive actions, ask-user findings),
   ...
   Keep that append to a single short line, exactly as rule 4 requires: firstmate's monitoring reads
   the last status line, so a multi-line append hides the decision from it. Put the full framing in
   your pane output or the PR body instead, which firstmate reads on wake:
   the options and their tradeoffs, your recommendation, and a one-line blast radius -
   what breaks if the call is wrong, and what would catch it if it does.
   That framing sharpens the decision and must never delay reporting it.
   When firstmate replies or a blocker clears and you resume, append \`resolved: {how it was decided or unblocked}\` (add the same \`[key=<slug>]\` if you opened it with one) so the decision or blocker is durably closed and does not keep resurfacing.
